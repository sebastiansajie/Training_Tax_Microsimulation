...
def remove_outliers_centile(df, field_name, lower_value=None, upper_value=None):
    ...
    if lower_value is not None:
        centile_value1 = min(df[field_name].quantile(lower_value/100).tolist())
    ...
    if upper_value is not None:
        centile_value2 = max(df[field_name].quantile(upper_value/100).tolist())
...
def my_qcut_equal_length(df, ranking_col, centile_length):
    ...
    for i in range(len(row_bin)-1):
        #print(i, row_bin[i])
        df['centile'] = np.where(((df[ranking_col]>row_bin[i][0])&
                                  (df[ranking_col]<=row_bin[i+1][0])),i+1,df['centile'])
    #df.loc[df.index[-1], 'centile'] = centile_length
    return df
    
def centile_summary(df, summary_col, ranking_col, centile_field):
    ...
    df6 = pd.concat([df0, df2, df3, df4, df5], axis=1)
    #print('df6', df6)
    return (df6)
